...
const api: WorkspaceApi = {
  ...
  memoryCreate: (input: NewMemoryInput) => ipcRenderer.invoke('memory:create', input),
  memorySave: (memory: MemoryRecord) => ipcRenderer.invoke('memory:save', memory),
  memoryList: () => ipcRenderer.invoke('memory:list'),
  memoryRemove: (memoryId) => ipcRenderer.invoke('memory:remove', memoryId),
  memoryClear: () => ipcRenderer.invoke('memory:clear'),
  memoryExport: () => ipcRenderer.invoke('memory:export'),
  messageCreate: (input: NewMessageInput) => ipcRenderer.invoke('message:create', input),
  messageSave: (message: MessageRecord) => ipcRenderer.invoke('message:save', message),
  messageList: () => ipcRenderer.invoke('message:list'),
  messageRemove: (messageId) => ipcRenderer.invoke('message:remove', messageId),
  conversationCreate: (conversation: ConversationRecord) =>
    ipcRenderer.invoke('conversation:create', conversation),
